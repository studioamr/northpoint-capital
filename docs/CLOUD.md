...
Ya con los usuarios creados, el pill de Aprobaciones pasa de `TRABAJANDO SOLO` a
...
- **Las firmas ya no se pisan.** Cada guardado lleva número de revisión y escribe sólo si
  ...
  misma tesis se suman: Pablo y Mateo pueden firmar al mismo tiempo sin borrarse.
- **Caja se fusiona de verdad.** `caja` (los ingresos y gastos reales — payouts cobrados,
  lo que costó operar) no estaba en la lista de campos que se fusionan por id: el socio
  que sincronizaba último se quedaba con su arreglo completo y borraba en silencio los
  movimientos de los demás. Corregido y probado con datos sintéticos.
...
|---|---|
| `TRABAJANDO SOLO` | Todavía no existen los usuarios, o nadie ha entrado con su correo |
